            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="box box-primary">
                        ...
                        <!-- /.box-header -->
                        <!-- form start -->
                        <form role="form" autocomplete="off" method="post" action="{{ url('/backend/culinaries/update') }}" enctype="multipart/form-data">
                            {{method_field('PUT')}}
                            <div class="box-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="hidden" class="form-control" id="kuliner_id" name="kuliner_id" value="{{$editKuliner->kuliner_id}}" readonly>
                                    <input type="text" class="form-control" id="nama_kuliner" name="nama_kuliner" placeholder="Culinary" value="{{ucwords($editKuliner->nama_kuliner)}}">
                                </div>
                                <div class="form-group">
                                    <label>City</label>
                                    <select class="form-control select2" style="width: 100%;" id="city_id" name="city_id">
                                        <option></option>
                                        @foreach($kota as $row)
                                            <option value="{{$row->kota_id}}" <?php echo ($editKuliner->kota_id == $row->kota_id) ? "selected" : "";?>>{{$row->nama_kota}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label>Address</label>
                                    <textarea class="form-control" rows="3" placeholder="Enter ..." id="address" name="address">{{ucfirst($editKuliner->alamat)}}</textarea>
                                </div>

                                <div class="form-group">
                                    <label>Description</label>
                                    <textarea class="form-control" rows="5" placeholder="Enter ..." id="description" name="description">{{ucfirst($editKuliner->deskripsi)}}</textarea>
                                </div>
